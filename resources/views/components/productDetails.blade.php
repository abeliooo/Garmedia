    ...
    <div class="container my-5">
        <div class="row">
            ...
            <div class="col-lg-7 ps-lg-0">
                ...
                <a href="{{ route('search', ['search' => $book->author]) }}"
                    class="lead text-muted text-decoration-none mb-3 d-block">{{ $book->author }}</a>
                ...
                <div class="mb-3">
                    <label class="form-label fw-bold">Formats:</label>
                    <div class="btn-group" role="group">
                        @php
                            $availableFormats = $book->formats ?? [];
                        @endphp

                        <input type="radio" class="btn-check" name="format" id="format-soft" autocomplete="off"
                            {{ in_array('soft cover', $availableFormats) ? '' : 'disabled' }}>
                        <label
                            class="btn {{ in_array('soft cover', $availableFormats) ? 'btn-outline-primary' : 'btn-outline-secondary disabled' }}"
                            for="format-soft">Soft Cover</label>

                        <input type="radio" class="btn-check" name="format" id="format-hard" autocomplete="off"
                            {{ in_array('hard cover', $availableFormats) ? '' : 'disabled' }}>
                        <label
                            class="btn {{ in_array('hard cover', $availableFormats) ? 'btn-outline-primary' : 'btn-outline-secondary disabled' }}"
                            for="format-hard">Hard Cover</label>
                    </div>
                </div>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <ul class="list-unstyled">
                                ...
                                <li class="mb-2"><strong>Release
                                        Date:</strong><br>{{ $book->release_date->format('d F Y') }}</li>
                            </ul>
                        </div>
                        <div class="col-md-6">
                            <ul class="list-unstyled">
